...
const MyAction = () => {
  ...
  if (!activeAction) {
    return (
      <div className="min-h-screen bg-background pattern-dots pb-24 flex items-center justify-center">
        <Card className="max-w-md p-12 text-center space-y-6 shadow-elevated">
          <div className="w-20 h-20 rounded-full bg-muted mx-auto flex items-center justify-center">
            <Sparkles className="w-10 h-10 text-muted-foreground" />
          </div>
          <div className="space-y-2">
            <h2 className="text-2xl font-bold text-foreground">No Active Action</h2>
            <p className="text-muted-foreground">Browse and select an action to begin your journey.</p>
          </div>
  ...
  if (showCompletion) {
    return (
      <div className="min-h-screen bg-background pattern-dots pb-24 flex items-center justify-center p-4">
        <Card className="max-w-2xl p-12 text-center space-y-8 shadow-elevated animate-scale-in">
          ...
          <div className="space-y-4">
            ...
            <h2 className="text-3xl font-bold text-foreground">Action Completed!</h2>
            <p className="text-xl text-gold font-semibold">+{barakahPoints} Barakah Points</p>
          </div>
          ...
          {reflection && (
            <div className="bg-card border border-border rounded-lg p-6 text-left">
              <h3 className="font-semibold text-foreground mb-2">Your Reflection:</h3>
          ...
          <div className="text-sm text-muted-foreground space-y-1">
            <p>You helped <span className="font-bold text-foreground">{activeAction.impactMetrics.people} people</span></p>
            <p>Impact duration: <span className="font-bold text-foreground">{activeAction.impactMetrics.duration}</span></p>
          </div>
      ...
      <header className="sticky top-0 z-40 bg-card/95 backdrop-blur-sm border-b border-border shadow-soft">
        <div className="max-w-4xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-3xl font-bold text-foreground">My Action</h1>
          ...
          <p className="text-sm text-muted-foreground mt-2">
            {completedSteps.filter(Boolean).length} of {activeAction.steps.length} steps completed
          </p>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 space-y-8">
        ...
        <Card className="p-6 shadow-elevated space-y-4">
          ...
          <div className="space-y-3 pt-4">
            <h3 className="font-semibold text-foreground">Steps to Complete:</h3>
        ...
        <Card className="p-6 shadow-soft space-y-3">
          <h3 className="font-semibold text-foreground">Reflection & Niyyah (Optional)</h3>
          <Textarea
            placeholder="What did you learn from this action? What was your intention?"
            value={reflection}
